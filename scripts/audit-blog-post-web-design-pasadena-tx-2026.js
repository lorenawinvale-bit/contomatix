 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-web-design-pasadena-tx-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-web-design-pasadena-tx-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-web-design-pasadena-tx-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-pasadena-tx-seo-2026.js, itself cloned from the recruiting
// series): this post is an honest single-company result for Pasadena, TX,
// so the focus keyword is "web design company Pasadena TX" (singular), not
// the "web design companies" plural phrasing used in multi-result city posts.
const FOCUS_KEYWORD_PATTERNS = [
  /top web design company (in )?pasadena,? tx/i,
  /web design company pasadena,? tx/i,
  /web design company in pasadena, tx/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "web design company in Pasadena, TX" (or close variant) near the start'
);
