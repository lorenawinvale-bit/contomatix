#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Sugar Land recruiting post variant.
 *
 ...
 * script) adapted for the "Top 5 Recruiting Companies in Sugar Land, TX"
 * post, following the same per-post-variant pattern as
 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-sugar-land-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-sugar-land-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-sugar-land-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /recruiting companies in sugar land/i,
  /recruiting companies sugar land/i,
  /top recruiting companies in sugar land/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in Sugar Land" (or close variant) near the start'
);
