 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-digital-marketing-deer-park-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-digital-marketing-deer-park-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-digital-marketing-deer-park-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-digital-marketing-conroe-2026.js): this copy hardcodes
// the digital-marketing phrasing for Deer Park, TX instead.
const FOCUS_KEYWORD_PATTERNS = [
  /digital marketing agencies deer park/i,
  /digital marketing agencies in deer park/i,
  /top digital marketing agencies in deer park/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "digital marketing agencies in Deer Park" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// This is a zero-verified-agencies post (same pattern as the League City, TX
// SEO zero-result post it's modeled on): there are no company homepage
// screenshots to embed, since no candidate passed verification. The base
// series script requires imgSrcMatches.length > 0, which doesn't fit a
// zero-result post that legitimately has no in-content <img> tags at all.
// So this copy passes cleanly on zero in-content images, and only fails if
// an <img> tag IS present but points at a file that doesn't exist on disk.
...
check(
  ...
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags — expected for a zero-verified-agencies post (featured SVG only)'
    : missingImages.length === 0
