...
const FOCUS_KEYWORD_PATTERNS = [
  /find competitor backlinks/i,
  /find competitors[' ]backlinks/i,
  /competitor backlink/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "backlink analysis tools" (or close variant) near the start'
);
...
check(
  'All in-content <img> references exist on disk (0 images OK for a text/educational guide)',
  missingImages.length === 0,
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags — expected for this guide (featured SVG only, matches link-building-for-dentists-2026 precedent)'
    : missingImages.length === 0
