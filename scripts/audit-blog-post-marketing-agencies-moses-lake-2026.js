 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-marketing-agencies-moses-lake-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-marketing-agencies-moses-lake-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-marketing-agencies-moses-lake-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-sugar-land-seo-2026.js): the recruiting-series script
// hardcodes "recruiting companies <city>" phrasing, which doesn't fit a
// marketing-agencies post, so this copy hardcodes the marketing-agencies
// phrasing for Moses Lake instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top marketing agencies in moses lake/i,
  /marketing agencies moses lake/i,
  /top marketing agencies moses lake/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top marketing agencies in Moses Lake" (or close variant) near the start'
);
