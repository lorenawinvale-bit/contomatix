#!/usr/bin/env node
/**
 * Blog post pre-publish audit script.
 *
 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-digital-marketing-moses-lake-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-digital-marketing-moses-lake-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-digital-marketing-moses-lake-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// ---------------------------------------------------------------------------
// This is a series-specific copy of the base audit script (same pattern as
// audit-blog-post-sugar-land-seo-2026.js and audit-blog-post-graphic-design-
// pasadena-tx-2026.js): this post covers digital marketing agencies
// specifically for Moses Lake, WA, so the focus keyword is hardcoded to that
// phrasing rather than the recruiting-series or SEO-companies phrasing used
// by other copies of this script.
const FOCUS_KEYWORD_PATTERNS = [
  /top digital marketing agencies in moses lake/i,
  /digital marketing agencies moses lake/i,
  /top digital marketing agencies moses lake/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top digital marketing agencies in Moses Lake" (or close variant) near the start'
);
...
if (imageExists && imagePath.toLowerCase().endsWith('.svg')) {
  ...
  try {
    ...
    const openTags = svgContent.match(/<([a-zA-Z][a-zA-Z0-9]*)(?:\s[^>]*)?(?<!\/)>/g) || [];
...
// ---------------------------------------------------------------------------
// Adapted for this post's format (same pattern as audit-blog-post-graphic-
// design-pasadena-tx-2026.js): both candidate companies' websites failed
// independent verification during this build — MOSEY Marketing's domain
// didn't resolve (DNS failure), and Motion Marketing's only discoverable
// domain couldn't be confirmed as belonging to the actual Moses Lake business
// (no address/city match anywhere on the live page, and its own Yelp listing
// names no website). Per the site's standing rule against fabricating or
// mis-attributing screenshots, no company-homepage screenshots are used in
// this post, and the featured SVG is the sole image. The generic script's
// version of this check fails outright when zero in-content <img> tags are
// present, which assumes a company-list structure with screenshots that
// doesn't apply to this honest-disclosure post. This version passes when
// either (a) no in-content images are referenced at all (SVG-only post,
// valid for this format), or (b) every referenced in-content image actually
// exists on disk.
...
check(
  'All in-content <img> references exist on disk (or none present — SVG-only post is valid for this format)',
  missingImages.length === 0,
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags referenced; both candidate company websites failed independent verification during this build, so the featured SVG is the sole image for this post'
    : missingImages.length === 0
...
// ---------------------------------------------------------------------------
// 12. Slug/title uniqueness against existing blog.json
// ---------------------------------------------------------------------------
