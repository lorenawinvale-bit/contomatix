 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-marketing-agencies-deer-park-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-marketing-agencies-deer-park-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-marketing-agencies-deer-park-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-marketing-agencies-tomball-2026.js and
// audit-blog-post-marketing-agencies-league-city-2026.js): the generic
// recruiting-series script hardcodes "recruiting companies <city>" phrasing,
// which doesn't fit a marketing-agencies post, so this copy hardcodes the
// marketing-agencies phrasing for Deer Park instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top marketing agencies deer park/i,
  /marketing agencies deer park/i,
  /top marketing agencies in deer park/i,
  /marketing agencies in deer park/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top marketing agencies in Deer Park" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// 11. Referenced in-content images exist on disk (zero-result-post aware)
// ---------------------------------------------------------------------------
// This is a zero-verified-companies post (same pattern as
// audit-blog-post-seo-league-city-2026.js, itself modeled on the Pasadena, TX
// graphic-design zero-result post): there are no company homepage screenshots
// to embed, since no company passed verification. The base series script
// requires imgSrcMatches.length > 0, which doesn't fit a zero-result post
// that legitimately has no in-content <img> tags at all. So this copy passes
// cleanly on zero in-content images, and only fails if an <img> tag IS
// present but points at a file that doesn't exist on disk.
...
check(
  'All in-content <img> references exist on disk (0 images OK for a zero-result post)',
  missingImages.length === 0,
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags — expected for a zero-verified-companies post (featured SVG only)'
    : missingImages.length === 0
