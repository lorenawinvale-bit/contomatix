 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-web-design-cypress-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-web-design-cypress-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-web-design-cypress-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-web-design-tomball-2026.js and audit-blog-post-seo-conroe-2026.js):
// this post is an honest "Top 1" verified-pick post for Cypress, TX web design
// specifically, so the focus keyword is "web design companies Cypress" / "top
// web design companies in Cypress, TX" rather than the SEO, digital-marketing,
// or graphic-design phrasing used by sister posts for other cities.
const FOCUS_KEYWORD_PATTERNS = [
  /top web design companies (in )?cypress/i,
  /web design companies cypress/i,
  /web design companies in cypress/i,
  /web design company (in )?cypress/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top web design companies in Cypress, TX" (or close variant) near the start'
);
