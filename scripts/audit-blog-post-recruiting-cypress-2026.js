#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Cypress recruiting post variant.
 *
 ...
 *
 * This is a copy of scripts/audit-blog-post-recruiting-conroe-2026.js
 * adapted for the "Top Recruiting Companies in Cypress, TX" post,
 * following the same per-post-variant pattern as the rest of the
 * recruiting series: the focus-keyword patterns are swapped for this
 * post's topic/city, and the "Best fit for:"/"Pricing:" separate-<p>-tag
 * check (added for the recruiting/graphic-design profile format) is
 * included.
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-cypress-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-cypress-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-cypress-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /recruiting companies in cypress/i,
  /recruiting companies cypress/i,
  /top recruiting companies in cypress/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in Cypress" (or close variant) near the start'
);
