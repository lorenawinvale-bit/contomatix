 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-seo-deer-park-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-seo-deer-park-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-seo-deer-park-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-seo-league-city-2026.js and audit-blog-post-seo-cypress-2026.js):
// the SEO-companies series hardcodes the "seo companies <city>" phrasing per
// city, so this copy hardcodes the SEO-companies phrasing for Deer Park, TX
// instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top seo companies deer park/i,
  /seo companies deer park/i,
  /top seo companies in deer park/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top SEO companies in Deer Park" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// This is a zero-verified-companies post (same pattern as the League City, TX
// zero-result post it's modeled on): there are no company homepage screenshots
// to embed, since no company passed verification. The base series script
// requires imgSrcMatches.length > 0, which doesn't fit a zero-result post that
// legitimately has no in-content <img> tags at all. So this copy passes
// cleanly on zero in-content images, and only fails if an <img> tag IS present
// but points at a file that doesn't exist on disk.
