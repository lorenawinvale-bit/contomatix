#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Deer Park recruiting post variant.
 *
 ...
 *
 * This is a copy of scripts/audit-blog-post-recruiting-league-city-2026.js
 * adapted for the "Top Recruiting Companies in Deer Park, TX" post: the
 * focus-keyword patterns are swapped for this post's topic/city, and the
 * "Best fit for:"/"Pricing:" separate-<p>-tag check (added for the
 * recruiting/graphic-design profile format) is included.
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-deer-park-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-deer-park-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-deer-park-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /recruiting companies in deer park/i,
  /recruiting companies deer park/i,
  /top recruiting companies in deer park/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in Deer Park" (or close variant) near the start'
);
