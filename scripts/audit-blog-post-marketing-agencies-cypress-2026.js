 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-marketing-agencies-cypress-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-marketing-agencies-cypress-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-marketing-agencies-cypress-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-marketing-agencies-the-woodlands-2026.js): the generic
// recruiting-series script hardcodes "recruiting companies <city>" phrasing,
// which doesn't fit a marketing-agencies post, so this copy hardcodes
// the marketing-agencies phrasing for Cypress instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top marketing agencies cypress/i,
  /marketing agencies cypress/i,
  /top marketing agencies in cypress/i,
  /marketing agencies in cypress/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top marketing agencies in Cypress, TX" (or close variant) near the start'
);
