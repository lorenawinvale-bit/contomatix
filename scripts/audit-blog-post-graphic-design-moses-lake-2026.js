#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Moses Lake graphic design post variant.
 *
 ...
 * script) adapted for the "Top 2 Verified Graphic Design Companies in Moses
 * Lake, WA" post, following the same per-post-variant pattern as
 * scripts/audit-blog-post-graphic-design-sugar-land-2026.js: the focus-keyword
 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-graphic-design-moses-lake-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-graphic-design-moses-lake-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-graphic-design-moses-lake-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /graphic design companies in moses lake/i,
  /graphic design companies moses lake/i,
  /top graphic design companies in moses lake/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "graphic design companies in Moses Lake" (or close variant) near the start'
);
