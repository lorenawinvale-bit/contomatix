 ...
 *
 * Usage:
 *   node scripts/audit-blog-post.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post.js <draft.json> [--write]');
  process.exit(1);
}
...
// ---------------------------------------------------------------------------
// Derive the target city from the draft (an explicit `focusKeyword` field wins;
// otherwise it's inferred from the slug, e.g. "top-recruiting-companies-deerfield-beach-2026"
// -> "deerfield beach"), so this check works for any city in the series, not just one.
// This is a series-specific copy of the base audit script (same pattern as
// audit-graphic-design-reno-2026.js and audit-blog-post-pleasanton.js): the
// recruiting-series script hardcodes "recruiting companies <city>" phrasing,
// which doesn't fit an SEO-companies post, so this copy hardcodes the
// SEO-companies / The Woodlands phrasing instead.
const FOCUS_KEYWORD_PATTERNS = [
  /seo companies the woodlands/i,
  /seo companies in the woodlands/i,
  /top seo companies in the woodlands/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "SEO companies The Woodlands" (or close variant) near the start'
);
