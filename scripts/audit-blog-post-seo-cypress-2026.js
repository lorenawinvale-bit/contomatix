 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-seo-cypress-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-seo-cypress-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-seo-cypress-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-seo-conroe-2026.js and audit-blog-post-pleasanton.js): the
// SEO-companies series hardcodes the "seo companies <city>" phrasing per
// city, so this copy hardcodes the SEO-companies phrasing for Cypress, TX
// instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top seo companies cypress/i,
  /seo companies cypress/i,
  /top seo companies in cypress/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top SEO companies in Cypress" (or close variant) near the start'
);
