#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Conroe recruiting post variant.
 *
 ...
 * script) adapted for the "Top 3 Recruiting Companies in Conroe, TX"
 * post, following the same per-post-variant pattern as
 * scripts/audit-blog-post-recruiting-sugar-land-2026.js: the focus-keyword
 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-conroe-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-conroe-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-conroe-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /recruiting companies in conroe/i,
  /recruiting companies conroe/i,
  /top recruiting companies in conroe/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in Conroe" (or close variant) near the start'
);
