 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-tomball-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-tomball-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-tomball-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-sugar-land-seo-2026.js and audit-blog-post-recruiting-moses-lake-2026.js):
// this copy hardcodes the "recruiting companies Tomball" phrasing.
const FOCUS_KEYWORD_PATTERNS = [
  /top recruiting companies in tomball/i,
  /recruiting companies tomball/i,
  /top recruiting companies tomball/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top recruiting companies in Tomball" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// Adapted for this post's format (same precedent as
// audit-blog-post-graphic-design-pasadena-tx-2026.js): neither candidate's
// homepage screenshot could be captured during this build. Express
// Employment Professionals' own site returned a bot-blocking 403 to both a
// headless-browser attempt and a direct connection check; Amerifen Solutions
// LLC has no reachable website at all (the closest-matching domain belongs
// to an unrelated insurance company). Rather than publish a broken or
// misattributed screenshot, this post relies solely on the featured SVG, and
// the check below is relaxed to accept zero in-content <img> tags as valid
// for this format while still failing if any referenced image is missing.
...
check(
  ...
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags referenced; both company screenshots were unobtainable (403 block / unreachable site), disclosed in the post, and the featured SVG is the sole image'
    : missingImages.length === 0
