 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-seo-league-city-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-seo-league-city-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-seo-league-city-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-seo-conroe-2026.js and audit-blog-post-seo-cypress-2026.js):
// the SEO-companies series hardcodes the "seo companies <city>" phrasing per
// city, so this copy hardcodes the SEO-companies phrasing for League City, TX
// instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top seo companies league city/i,
  /seo companies league city/i,
  /top seo companies in league city/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top SEO companies in League City" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// This is a zero-verified-companies post (same pattern as the Pasadena, TX
// graphic-design zero-result post it's modeled on): there are no company
// homepage screenshots to embed, since no company passed verification. The
// base series script requires imgSrcMatches.length > 0, which doesn't fit a
// zero-result post that legitimately has no in-content <img> tags at all
// (matching how graphic-design-companies-pasadena-tx-2026 itself ships with
// zero inline images, only the featured SVG). So this copy passes cleanly on
// zero in-content images, and only fails if an <img> tag IS present but
