 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-graphic-design-deer-park-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-graphic-design-deer-park-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-graphic-design-deer-park-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-graphic-design-cypress-2026.js): the generic recruiting-series
// script hardcodes "recruiting companies <city>" phrasing, which doesn't fit a
// graphic-design-companies post, so this copy hardcodes the graphic-design
// phrasing for Deer Park instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top graphic design companies deer park/i,
  /graphic design companies deer park/i,
  /top graphic design companies in deer park/i,
  /graphic design companies in deer park/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top graphic design companies in League City" (or close variant) near the start'
);
...
// ---------------------------------------------------------------------------
// Adapted for this post's format: unlike the standard "Top N Companies"
// posts, this is an honest zero-result explainer with no company-homepage
// screenshots to illustrate a ranked list, so the featured SVG is the sole
// image. The generic script's version of this check fails outright when zero
// in-content <img> tags are present, which assumes a company-list structure
// that doesn't apply here (same adaptation as
// audit-blog-post-graphic-design-pasadena-tx-2026.js). This version passes
// when either (a) no in-content images are referenced at all (SVG-only post,
...
check(
  ...
  imgSrcMatches.length === 0
    ? 'no in-content <img> tags referenced; the featured SVG is the sole image for this zero-result explainer post'
    : missingImages.length === 0
