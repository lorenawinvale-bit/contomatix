#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — League City recruiting post variant.
 *
 ...
 * adapted for the "Top 2 Recruiting Companies in League City, TX"
 * post: the focus-keyword patterns are swapped for this post's topic/city,
 * and the "Best fit for:"/"Pricing:" separate-<p>-tag check (added for the
 * recruiting/graphic-design profile format) is included.
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-league-city-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-league-city-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-league-city-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /recruiting companies in league city/i,
  /recruiting companies league city/i,
  /top recruiting companies in league city/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in League City" (or close variant) near the start'
);
