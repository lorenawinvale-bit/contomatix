 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-recruiting-pasadena-tx-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-recruiting-pasadena-tx-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-recruiting-pasadena-tx-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-recruiting-moses-lake-2026.js): this post is an 8-result
// recruiting-companies roundup for Pasadena, TX, so the focus keyword is
// "recruiting companies Pasadena TX" (plural), matching the series pattern
// used for other cities in data/blog.json's "Recruiting & Staffing" category.
const FOCUS_KEYWORD_PATTERNS = [
  /top recruiting companies (in )?pasadena,? tx/i,
  /recruiting companies (in )?pasadena,? tx/i,
  /recruiting companies pasadena,? tx/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "recruiting companies in Pasadena, TX" (or close variant) near the start'
);
