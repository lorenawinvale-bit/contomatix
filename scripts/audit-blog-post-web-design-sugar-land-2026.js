#!/usr/bin/env node
/**
 * Blog post pre-publish audit script — Sugar Land web design post variant.
 *
 ...
 * adapted for the "Top 4 Web Design Companies in Sugar Land, TX" post: the
 * focus-keyword patterns are swapped for this post's topic/city. The
 * "Best fit for:"/"Pricing:" separate-<p>-tag check is retained since this
 * post uses the same profile format.
 *
 * Usage:
 *   node scripts/audit-blog-post-web-design-sugar-land-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-web-design-sugar-land-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-web-design-sugar-land-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
const FOCUS_KEYWORD_PATTERNS = [
  /web design companies in sugar land/i,
  /web design companies sugar land/i,
  /top web design companies in sugar land/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "web design companies in Sugar Land" (or close variant) near the start'
);
