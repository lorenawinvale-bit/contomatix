 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-graphic-design-pasadena-tx-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-graphic-design-pasadena-tx-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-graphic-design-pasadena-tx-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// audit-blog-post-pasadena-tx-seo-2026.js): this post is an honest
// zero-result finding for Pasadena, TX graphic design companies, so the
// focus keyword is "graphic design companies Pasadena TX" rather than the
// "top N companies" phrasing used in the standard list-format posts.
const FOCUS_KEYWORD_PATTERNS = [
  /graphic design companies (in )?pasadena,? tx/i,
  /graphic design company pasadena,? tx/i,
  /graphic design companies in pasadena, tx/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "graphic design companies in Pasadena, TX" (or close variant) near the start'
);
...
// that doesn't apply here. This version passes when either (a) no in-content
// images are referenced at all (SVG-only post, valid for this format), or
// (b) every referenced in-content image actually exists on disk.
