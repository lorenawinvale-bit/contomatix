 ...
 *
 * Usage:
 *   node scripts/audit-blog-post-seo-conroe-2026.js <draft.json>              (dry run, default)
 *   node scripts/audit-blog-post-seo-conroe-2026.js <draft.json> --write      (append to blog.json)
 *
...
if (!draftArg) {
  console.error('Usage: node scripts/audit-blog-post-seo-conroe-2026.js <draft.json> [--write]');
  process.exit(1);
}
...
// SEO-companies phrasing for Conroe instead.
const FOCUS_KEYWORD_PATTERNS = [
  /top seo companies conroe/i,
  /seo companies conroe/i,
  /top seo companies in conroe/i,
];
...
check(
  ...
  boldedInFirst10Pct,
  boldedInFirst10Pct ? 'OK' : 'expected a <strong> tag matching "top SEO companies in Conroe" (or close variant) near the start'
);
