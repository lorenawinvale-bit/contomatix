...
// Every public-facing route (listing, sitemap, related posts, etc.) should
// use this instead of getAll() — a post dated in the future is real data
// (visible to the admin dashboard) but not yet public. Because this checks
// the date on every request rather than at build/deploy time, a scheduled
// post goes live on its own the moment its date arrives, no redeploy needed.
function isPublished(post) {
  return post.date <= new Date().toISOString().slice(0, 10);
}

function getPublished() {
  return getAll().filter(isPublished);
}
...
function getRelated(slug, count) {
  count = count || 3;
  // Draw only from published posts — otherwise a live post could link out
  // to (and reveal the existence of) a post scheduled for the future.
  const all = getPublished();
  const post = all.find(p => p.slug === slug) || getAll().find(p => p.slug === slug);
  if (!post) return [];
...
module.exports = { getAll, getPublished, isPublished, getBySlug, create, update, remove, slugify, getRelated };
